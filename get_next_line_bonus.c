...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/17 21:04:49 by mmaila            #+#    #+#             */
/*   Updated: 2023/11/23 13:17:23 by mmaila           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	readbuf[0] = '\0';
	chyata[fd] = chyatacheck(chyata[fd], &readbuf);
	if (!chyata[fd])
		return (NULL);
	buf = NULL;
	buf = readfile(&chyata[fd], buf, &readbuf, fd);
	if (!buf)
		return (NULL);
	return (buf);
}
